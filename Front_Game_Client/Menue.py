...
logo = None
try:
    if os.path.exists("./Front_Game_Client/logo.jpeg"):
        logo = pygame.image.load("./Front_Game_Client/logo.jpeg")
        logo = pygame.transform.scale(logo, (150, 150))
        print("Logo loaded successfully.")
    else:
        print("Logo file not found at ./BackEnd/logo.jpeg")
...
def menu_screen():
    global search_text, search_result, message
    ...
    while True:
        ...
        for i, f in enumerate(friends[:5]):
            y = 170 + i * 50
            card_rect = pygame.Rect(40, y, 360, 40)
            pygame.draw.rect(screen, (230, 245, 255), card_rect, border_radius=10)
            draw_text(f['Username'], 80, y + 10)
            dot_color = (0, 200, 0) if f.get("Online") else (200, 0, 0)
            pygame.draw.circle(screen, dot_color, (55, y + 20), 8)

        draw_text("Requests:", 500, 140)
        for i, sender in enumerate(friend_requests[:5]):
            y = 170 + i * 60
            card_rect = pygame.Rect(500, y, 360, 50)
            pygame.draw.rect(screen, (255, 240, 240), card_rect, border_radius=10)
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if acc_btn.collidepoint(event.pos):
                        accept_friend(sender["id"])
                        fetch_friend_data()
                    elif rej_btn.collidepoint(event.pos):
        ...
        start_btn = draw_button("Start Game", 350, 580, 200, 50)

        req_btn = None
        if search_result == "not_found":
            draw_text("User not found.", 500, 500, RED)
        elif search_result == "already_friends":
            draw_text("Already your friend or request already sent.", 500, 500, ORANGE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if search_btn.collidepoint(event.pos):
                    ...
                    message = ""
                if req_btn and req_btn.collidepoint(event.pos):
